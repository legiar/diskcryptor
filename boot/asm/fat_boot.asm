...
; this code ripped from ReactOS project
...
org 7c3Eh
	...
	mov es,ax								; Make ES correct
	     ...
	     ;   cmp BYTE [bp+BootDrive], 0xff  ; If they have specified a boot drive then use it
...
CalcDriveSize:
		...
		mov   [bp-BiosCHSDriveSize],eax


	; Now we must find our way to the first sector of the root directory
	xor ax,ax
		xor cx,cx
	mov al,[bp+NumberOfFats]		   ; Number of fats
	mul WORD [bp+SectorsPerFat]		   ; Times sectors per fat
	add ax,WORD [ bp+HiddenSectors]
	adc dx,WORD [ bp+HiddenSectors+2]   ; Add the number of hidden sectors
	add ax,WORD [ bp+ReservedSectors]   ; Add the number of reserved sectors
	adc dx,cx								; Add carry bit
		mov WORD [ bp-DataAreaStartLow],ax  ; Save the starting sector of the root directory
		mov WORD [ bp-DataAreaStartHigh],dx ; Save it in the first 4 bytes before the boot sector
		mov si,WORD [ bp+MaxRootEntries]    ; Get number of root dir entries in SI
	...
	mov bx,[ bp+BytesPerSector]
	add ax,bx
	dec ax
	div bx									; Divided by the size of a sector
		; AX now has the number of root directory sectors

		add [ bp-DataAreaStartLow],ax		    ; Add the number of sectors of the root directory to our other value
		adc [ bp-DataAreaStartHigh],cx		    ; Now the first 4 bytes before the boot sector contain the starting sector of the data area
...
SearchRootDirSector:
		...
		add  di, +0x20						    ; Increment DI by the size of a directory entry
...
FoundFreeLoader:
	...
	; ES:DI has dir entry (ES:DI == 07E0:XXXX)
	 mov  ax,WORD [es:di+1ah]				 ; Get start cluster
		...
		mov  WORD [ bp-ReadSectorsOffset],ReadSectors		    ; Save the address of ReadSectors
		mov  WORD [ bp-ReadClusterOffset],ReadCluster		     ; Save the address of ReadCluster
		mov  WORD [ bp-PutCharsOffset],PutChars 			    ; Save the address of PutChars
...
ReadCluster:
	...
	xor   ch,ch
	mov   cl,BYTE [ bp+SectsPerCluster]
...
ReadSectors:
		...
		add ax,cx
		adc dx, 0
		cmp dx,WORD [ bp-BiosCHSDriveSizeHigh]	    ; Check if they are reading a sector within CHS range
		ja  ReadSectorsLBA							; No - go to the LBA routine
		jb  ReadSectorsCHS							; Yes - go to the old CHS routine
		cmp ax,WORD [ bp-BiosCHSDriveSizeLow]	    ; Check if they are reading a sector within CHS range
...
;               jz   PrintDiskError                                             ; Bit 0, extended disk access functions (AH=42h-44h,47h,48h) supported
	...
	push bx
	mov  bx,es
	add  bx, 20h						    ; Increment read buffer for next sector
	mov  es,bx
	pop  bx
...
ReadSectorsCHSLoop:
	...
	xchg ax,dx
	xor  dx,dx
	div  WORD [ bp+SectorsPerTrack]
	xchg ax,cx		      
	div  WORD [ bp+SectorsPerTrack]    ; Divide logical by SectorsPerTrack
	inc  dx 		       ; Sectors numbering starts at 1 not 0
	xchg cx,dx
	div  WORD [ bp+NumberOfHeads]	   ; Number of heads
...
msgFreeLdr   db 'DCLDR not found',0dh,0ah,0
