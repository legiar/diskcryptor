...
static void dc_save_enc_state(dev_hook *hook, int finish)
{
	...
	if (finish != 0)
	{
		...
		if (hook->tmp_header.flags & F_REENCRYPT) {
			burn(hook->tmp_header.key_2, DISKKEY_SIZE); 
			...
			case S_INIT_RE_ENC:
				{
					...
					/* swap keys */
					memcpy(tmp_key, &hook->dsk_key, sizeof(xts_key));
					memcpy(&hook->dsk_key, hook->tmp_key, sizeof(xts_key));
					memcpy(hook->tmp_key, tmp_key, sizeof(xts_key));
			...
			case S_CONTINUE_RE_ENC:
				{
					...
					xts_set_key(hook->tmp_header.key_2, hook->tmp_header.alg_2, tmp_key);
